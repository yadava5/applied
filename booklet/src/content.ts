...
export const PROOF = {
  ...
  tests: {
    ...
    body:
      "Correctness is not asserted, it is enforced. The backend suite runs 1,309 tests, all passing — including the twenty-one Postgres row-level-security tests, which provision their own postgres:16 through testcontainers instead of skipping — and two CI gates re-run the classifier evaluation on every change, refusing to merge if macro-F1 falls below 0.95. A suite only checks what somebody thought to check, so there is a second instrument: 17,020 generated messages across 35 families over 9,180 companies — every employer and role invented, six of those families phrased in wordings transcribed from mail that actually arrived — 18% of them adversarial by construction, driven through the whole sync end to end — classify, roll up, upsert, persist the review queue, then read the board back out of the tables, replayed in day-sized batches because a real sync is a delta and not a whole mailbox. 15,657 come out correct (91.99%), 343 wrong, 1020 abstained. No message lands on another application's card: 9,172 cards, 0 splits, 0 merges, 0 misrouted review, 0 updates on a card that was not theirs. Two messages that should mint nothing do mint a card, and that is the exception rather than a rounding of it. What the corpus could not see until it ran the review path is that 8 messages about real applications reach no card, no queue and no counter at all.",
    stats: [
      { value: "1,309", label: "tests · backend suite", note: "0 failed · 2026-08-22" },
      { value: "17,020", label: "messages · adversarial corpus", note: "35 families · 9,180 companies" },
      { value: "91.99%", label: "correct · stress corpus", note: "343 wrong · 1020 abstained" },
    ],
    honest:
      "Provenance: 1,309 is what `PYTHONPATH=. pytest tests -q --ignore=tests/test_setfit_model.py --ignore=tests/test_evaluate_classifier.py` passes from backend/, run 2026-08-22 on Python 3.11.14 with a Docker daemon available — without Docker and without JOBTRACKER_TEST_PG_ADMIN_URL the twenty-one row-level-security tests skip, and database-level tenant isolation is then simply unverified on that run. The corpus figure carries the worse condition, and printing it is the whole point: 102 of the 343 wrong verdicts sit above the 0.85 auto-file gate and are stated to the reader as fact, while 200 fall below it and are held for a person to settle. That ratio is the number worth watching, not the total — 50 wrong verdicts all auto-filed would be a worse product than 343 with 102 auto-filed. It read 464 of 464 above the gate on the morning of 2026-08-22, before the classifier stopped scoring quoted history and a reply's copied subject as the message's own words. And 18% of that mail is adversarial by construction, so 91.99% is behaviour under stress, not the accuracy an inbox would produce. The figure a reader should weigh against it is the one the corpus could not measure until it began running the review path: 8 of those messages are about real applications and reach nothing at all, which no accuracy number expresses. The test figure was 305 on 2026-08-06; the suite has grown, it has not been re-scoped.",
    handoffQuote:
