...
export const PROOF = {
  ...
  tests: {
    ...
    headline: "3,163 tests, and 14,540 messages written to break the classifier.",
    body:
      "Correctness is not asserted, it is enforced. The backend suite runs 3,163 tests, 3,153 of them passing and 10 expected failures — including the 25 Postgres row-level-security tests, which provision their own postgres:16 through testcontainers instead of skipping — and two CI gates re-run the classifier evaluation on every change, refusing to merge if macro-F1 falls below 0.95. A suite only checks what somebody thought to check, so there is a second instrument: 18,980 generated messages across 45 families over 8,880 companies — every employer invented, six of those families phrased in wordings transcribed from mail that actually arrived and one carrying job titles and locations taken from public Greenhouse job boards, six of its eight titles and all five locations byte-for-byte and the other two titles trimmed of a queue marker and a trailing space — 18% of them adversarial by construction, driven through the whole sync end to end — classify, roll up, upsert, persist the review queue, then read the board back out of the tables, replayed in day-sized batches because a real sync is a delta and not a whole mailbox. 17,550 come out correct (92.47%), 144 wrong, 1286 abstained. No message lands on another application's card: 10,058 cards, 0 merges, 0 misrouted review, 0 updates on a card that was not theirs. 0 applications are split across two cards; that number is kept separate from merges because a split is the milder failure, visible to the user rather than silent. Zero messages that should mint nothing do mint a card, and that is the exception rather than a rounding of it. 60 messages about real applications reach no card, no queue and no counter at all, and every one of them arrives on a card the user has dismissed by hand — a dismissal is final by design, so the sync skips the row before it stores anything. That figure read 0 until the generator learned to dismiss an application at all, which is the difference between a guarantee and a state no fixture could construct.",
    stats: [
      { value: "3,163", label: "tests · backend suite", note: "0 failed · 2026-09-05" },
      { value: "18,980", label: "messages · adversarial corpus", note: "45 families · 8,880 companies" },
      { value: "92.47%", label: "correct · stress corpus", note: "144 wrong · 1286 abstained" },
    ],
    honest:
      "Provenance: 3,163 is what `cd backend && pytest tests -q --cov=jobtracker --cov-report=json:.readme-facts-coverage.json --cov-report=term` collects, recorded 2026-09-05 on Darwin-arm64, Python 3.11.14 (`backend/.venv311`) with a Docker daemon available. That figure is not typed in here: it is read out of `docs/readme-facts.json` — the same recording the README publishes — by `scripts/readme_facts.py`, whose `--check` fails the build when this page and that artifact disagree, and which refuses to record at all unless the run was whole: Docker reachable, nothing skipped, suite green. It moves only when someone re-runs `--record`, so between recordings it describes an earlier tree, and `--check` prints how far the suite has moved since. Without Docker and without JOBTRACKER_TEST_PG_ADMIN_URL the 25 row-level-security tests skip, and database-level tenant isolation is then simply unverified on that run. The corpus figure carries the worse condition, and printing it is the whole point: 0 of the 144 wrong verdicts sit above the 0.85 auto-file gate and are stated to the reader as fact, while 144 fall below it and are held for a person to settle. That ratio is the number worth watching, not the total — 50 wrong verdicts all auto-filed would be a worse product than 144 with 0 auto-filed. It read 464 of 464 above the gate on the morning of 2026-08-22, before the classifier stopped scoring quoted history and a reply's copied subject as the message's own words. And 18% of that mail is adversarial by construction, so 92.47% is behaviour under stress, not the accuracy an inbox would produce. That figure moved DOWN by 0.88 points on 2026-09-06 and not because the classifier changed: the harness had been handing it uncapped bodies, so the 320-message family built for the product's own 4,000-character cap was graded on text the product cannot deliver, and 160 verdicts that counted as correct now abstain. It then moved UP by 1.10 points on 2026-09-07, and again not because the classifier changed: no engine file moved. The `quoted-history` family's reply was worded \"we would love to set up a conversation\", and the rules engine's volition pattern wants \"would like to\" — across the 110 externally-provenanced interview cases in the other corpus, `like to` appears 63 times and `love to` none, so 200 messages had been counted wrong for using language no real invitation uses. Worse than the 200: the case came out wrong whether the quote-stripping under test was active or not, so that family had been grading the thing it exists for not at all. Read both moves the same way — the engine has been still while the measurement got less wrong in one direction and then the other. Wrong did not move, none of the 160 was ever stated as fact, and every accuracy figure this page published before that date overstates by about the same amount. The figure a reader should weigh against it is the one the corpus could not measure until it began running the review path: 60 of those messages are about real applications and reach nothing at all, which no accuracy number expresses. The test figure was 305 on 2026-08-06 and the suite has grown since. The command behind it was re-scoped on 2026-08-29: it used to name a narrower run that ignored two model modules, and the number under it was transcribed by hand and had gone stale by 469 tests before anything noticed. It now names the recorded run, which is why this figure moved by more than the tests that were added.",
    handoffQuote:
