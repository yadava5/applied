...
export const PROOF = {
  ...
  tests: {
    ...
    headline: "1,783 tests, and 14,540 messages written to break the classifier.",
    body:
      "Correctness is not asserted, it is enforced. The backend suite runs 1,783 tests, all passing — including the twenty-one Postgres row-level-security tests, which provision their own postgres:16 through testcontainers instead of skipping — and two CI gates re-run the classifier evaluation on every change, refusing to merge if macro-F1 falls below 0.95. A suite only checks what somebody thought to check, so there is a second instrument: 17,260 generated messages across 37 families over 8,020 companies — every employer and role invented, six of those families phrased in wordings transcribed from mail that actually arrived — 18% of them adversarial by construction, driven through the whole sync end to end — classify, roll up, upsert, persist the review queue, then read the board back out of the tables, replayed in day-sized batches because a real sync is a delta and not a whole mailbox. 15,886 come out correct (92.04%), 361 wrong, 1013 abstained. No message lands on another application's card: 9,252 cards, 0 merges, 0 misrouted review, 0 updates on a card that was not theirs. 0 applications are split across two cards; that number is kept separate from merges because a split is the milder failure, visible to the user rather than silent. Zero messages that should mint nothing do mint a card, and that is the exception rather than a rounding of it. What the corpus could not see until it ran the review path is that 0 messages about real applications reach no card, no queue and no counter at all — a figure that read 610 the day it first became measurable.",
    stats: [
      { value: "1,783", label: "tests · backend suite", note: "0 failed · 2026-08-28" },
      { value: "17,260", label: "messages · adversarial corpus", note: "37 families · 8,020 companies" },
      { value: "92.04%", label: "correct · stress corpus", note: "361 wrong · 1013 abstained" },
    ],
    honest:
      "Provenance: 1,783 is what `cd backend && pytest tests -q --cov=jobtracker --cov-report=json:.readme-facts-coverage.json --cov-report=term` collects and passes, recorded 2026-08-28 on Darwin-arm64, Python 3.11.14 (`backend/.venv311`) with a Docker daemon available. That figure is not typed in here: it is read out of `docs/readme-facts.json` — the same recording the README publishes — by `scripts/readme_facts.py`, whose `--check` fails the build when this page and that artifact disagree, and which refuses to record at all unless the run was whole: Docker reachable, nothing skipped, suite green. It moves only when someone re-runs `--record`, so between recordings it describes an earlier tree, and `--check` prints how far the suite has moved since. Without Docker and without JOBTRACKER_TEST_PG_ADMIN_URL the twenty-one row-level-security tests skip, and database-level tenant isolation is then simply unverified on that run. The corpus figure carries the worse condition, and printing it is the whole point: 72 of the 361 wrong verdicts sit above the 0.85 auto-file gate and are stated to the reader as fact, while 200 fall below it and are held for a person to settle. That ratio is the number worth watching, not the total — 50 wrong verdicts all auto-filed would be a worse product than 361 with 72 auto-filed. It read 464 of 464 above the gate on the morning of 2026-08-22, before the classifier stopped scoring quoted history and a reply's copied subject as the message's own words. And 18% of that mail is adversarial by construction, so 92.04% is behaviour under stress, not the accuracy an inbox would produce. The figure a reader should weigh against it is the one the corpus could not measure until it began running the review path: 0 of those messages are about real applications and reach nothing at all, which no accuracy number expresses. It read 610 the day the review path was first replayed. The test figure was 305 on 2026-08-06 and the suite has grown since. The command behind it was re-scoped on 2026-08-29: it used to name a narrower run that ignored two model modules, and the number under it was transcribed by hand and had gone stale by 469 tests before anything noticed. It now names the recorded run, which is why this figure moved by more than the tests that were added.",
    handoffQuote:
