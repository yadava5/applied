...
export const PROOF = {
  ...
  tests: {
    ...
    headline: "1,306 tests, and 10,040 messages written to break them.",
    body:
      "Correctness is not asserted, it is enforced. The backend suite runs 1,306 tests, all passing — including the twenty-one Postgres row-level-security tests, which provision their own postgres:16 through testcontainers instead of skipping — and two CI gates re-run the classifier evaluation on every change, refusing to merge if macro-F1 falls below 0.95. A suite only checks what somebody thought to check, so there is a second instrument: 10,040 invented messages across 18 families over 5,670 companies, 18% of them adversarial by construction, driven end to end — classify, roll up, upsert, then read the board back out of the tables, replayed in day-sized batches because a real sync is a delta and not a whole mailbox. 8,930 come out correct (88.94%), 473 wrong, 637 abstained. The board is clean: 5,687 cards, 0 splits, 0 merges, 0 noise, 0 misrouted review.",
    stats: [
      { value: "1,306", label: "tests · backend suite", note: "0 failed · 2026-08-22" },
      { value: "10,040", label: "messages · adversarial corpus", note: "18 families · 5,670 companies" },
      { value: "88.94%", label: "correct · stress corpus", note: "473 wrong · 637 abstained" },
    ],
    honest:
      "Provenance: 1,306 is what `PYTHONPATH=. pytest tests -q --ignore=tests/test_setfit_model.py --ignore=tests/test_evaluate_classifier.py` passes from backend/, run 2026-08-22 on Python 3.11.14 with a Docker daemon available — without Docker and without JOBTRACKER_TEST_PG_ADMIN_URL the twenty-one row-level-security tests skip, and database-level tenant isolation is then simply unverified on that run. The corpus figure carries the worse condition, and printing it is the whole point: every one of the 473 wrong verdicts sits above the 0.85 auto-file gate. There is no wrong-but-hedged verdict in ten thousand messages — the review queue catches the classifier being unsure, and has never once caught it being wrong. And 18% of that mail is adversarial by construction, so 88.94% is behaviour under stress, not the accuracy an inbox would produce. The test figure was 305 on 2026-08-06; the suite has grown, it has not been re-scoped.",
    handoffQuote:
