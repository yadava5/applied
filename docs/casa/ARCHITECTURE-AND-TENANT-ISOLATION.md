...
(`backend/jobtracker/cloud/applications.py:3901`, the copy itself at `:3920`).
...
- **`training_data` has no foreign key to `emails`.** `training_data.email_id`
  ...
  (`backend/jobtracker/cloud/applications.py:3873`). One of the 7 populated rows
...
(`backend/jobtracker/database/models.py:1164-1171`). Two features need a
