...
5. **Server-side narrowing before any message is transferred.** Applied builds
   ...
   `gmail_client.py:1186-1223`), and the default read is `in:inbox` plus a
   ...
   - **Three paths read `in:anywhere`**, which includes archived mail. Sent
     ...
     `in:anywhere -in:sent` (`gmail_client.py:1220`), because the first windowed
     ...
     *destructive* scan — it can delete application rows whose mail no longer
     ...
     (`_HISTORY_EXCLUDED_LABELS`, `gmail_client.py:1778`, and the label filter
     itself at `:1910-1911`).
