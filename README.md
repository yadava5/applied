...
- **Tenant isolation enforced by Postgres, live in production.** Eight tenant tables carry `ENABLE` + `FORCE ROW LEVEL SECURITY` with four policies each — **32 policies** — and production connects as `jobtracker_app`, a `NOSUPERUSER NOBYPASSRLS` role. Thirteen tests drive the real connection machinery against a real Postgres, and CI fails the build if they *skip*.
...
| **Database isolation** | pytest + testcontainers / CI service container | 13 RLS enforcement tests against real Postgres |
