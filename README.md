...
<p align="center">
  <img src="https://img.shields.io/badge/tests-508%20collected%20%C2%B7%200%20skipped-brightgreen" alt="Tests">
...
- **Fixture demo** — the full UI on synthetic data at [`/demo`](https://getapplied.vercel.app/demo), no login. Layer 1 recomputes **live in the browser** there via `apps/web/lib/demo/rulesLayer.ts`, a port of the same 201 patterns; layers 2 and 3 are precomputed, because the app's CSP forbids the WASM eval and CDN fetch Transformers.js needs.
...
flowchart TB
    ...
    R{{"1 · rules<br/>201 regex patterns over 7 categories<br/>106 strong · 26 weak · 69 negative<br/>hand-written · classifier/rules.py"}}
...
Thresholds are `CONFIDENCE_AUTO = 0.85` and `CONFIDENCE_MIN_CLASSIFICATION = 0.70`, both defined in `backend/jobtracker/classifier/hybrid.py`. The 201 patterns are counted at their definition site — the `PATTERNS` dict in `classifier/rules.py` — not at any call site.
...
| **Testing** | Playwright 1.48+ (13 spec files under `apps/web/tests/e2e/`) |
...
| Category | Technologies |
| --- | --- |
| **Layer 1** | Hand-written regex engine, 201 patterns across 7 categories |
...
**508 tests collected, 0 skipped.** These figures were recorded on 2026-08-11 by `python3 scripts/readme_facts.py --record`, which runs `pytest tests -q --cov=jobtracker` in the project's Python 3.11.14 venv and writes `docs/readme-facts.json`; `--check` fails the build when this page and that artifact disagree. The count was first published from commit `37dd805` and corrected in `5b895d8`. It has grown since: a static parse counts 459 `test_*` functions across 31 modules at HEAD, against 300 across 25 modules at `37dd805` — the tests added with the sync-cursor, recoverable-removal, company-matching, stage-vocabulary, application-identity, reopen-after-rejection and RLS work, three of which brought their own module. The remaining difference from 508 is parametrization. CI reruns the suite with `--cov` on every push, so the current number lands in a public run log rather than resting on this sentence.
...
**Coverage**, from the same run: **57.61%** overall — 9,194 statements, 3,897 missed. The distribution matters more than the total. `jobtracker/cloud`, the code that actually deploys, is at **87.5%**; `auth` 80.5%; `database` 77.2%. What pulls the average down is `jobtracker/scripts` at 2,240 statements and 33.7%, of which eight modules no test imports account for 1,006 statements at 0%.
...
| **Web e2e** | Playwright | 13 spec files — landing, demo, dashboard, inbox, settings, import, connect, beta, shell, navigation, smoke, file-application, production |
...
- **The rules engine.** 201 regex patterns across 7 categories (106 strong, 26 weak, 69 negative), the scoring weights (strong +3, +6 in subject; weak +1, +2; negative −5), the margin-to-confidence tiers, and the ATS-domain boost. Ported byte-for-byte to JavaScript in `apps/web/lib/demo/rulesLayer.ts` and to `ml/browser/site/app.js`.
...
│   │   ├── classifier/      # rules.py (201 patterns) · embeddings.py · setfit_model.py · hybrid.py
