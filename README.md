...
<p align="center">
  <img src="https://img.shields.io/badge/tests-305%20collected%20%C2%B7%200%20skipped-brightgreen" alt="Tests">
...
- **Tenant isolation enforced by Postgres, live in production.** Eight tenant tables carry `ENABLE` + `FORCE ROW LEVEL SECURITY` with four policies each — **32 policies** — and production connects as `jobtracker_app`, a `NOSUPERUSER NOBYPASSRLS` role. Ten tests drive the real connection machinery against a real Postgres, and CI fails the build if they *skip*.
...
**Coverage**, from the same run: **53.19%** overall — 8,209 statements, 3,843 missed. The distribution matters more than the total. `jobtracker/cloud`, the code that actually deploys, is at **82.2%**; `auth` 80.5%; `database` 76.6%. What pulls the average down is `jobtracker/scripts` at 2,240 statements and 33.7%, of which eight modules no test imports account for 1,006 statements at 0%.
...
| **Database isolation** | pytest + testcontainers / CI service container | 10 RLS enforcement tests against real Postgres |
...
│   ├── alembic/versions/    # 7 revisions incl. the RLS + InitPlan-hoist migrations
