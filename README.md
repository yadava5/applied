...
**Coverage**, from the same run: **55.88%** overall — 8,964 statements, 3,955 missed. The distribution matters more than the total. `jobtracker/cloud`, the code that actually deploys, is at **82.9%**; `auth` 80.5%; `database` 77.0%. What pulls the average down is `jobtracker/scripts` at 2,240 statements and 33.7%, of which eight modules no test imports account for 1,006 statements at 0%.
