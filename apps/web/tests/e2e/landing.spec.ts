 ...
 * promoted here from `/landing-b` once the owner chose it. It replaced a
 * project-shaped case study whose own spec (the eight tests that used to be
 * this file) went with the page it gated; this file is `/`'s gate now.
 *
 * WHY THIS FILE EXISTS. The suite it replaced asserted copy — headlines,
 * door labels, counted-up figures — and the only automated cover this page
 * ever had was `tests/unit/landing-variants.test.mjs`, which is a
