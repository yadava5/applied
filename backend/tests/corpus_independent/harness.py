...
async def replay(session, verdicts: list[Verdict]) -> Replay:
    ...
    for day in sorted(by_day):
        ...
        for slot in sorted(per_user):
            ...
            # THE THIRD BOARD-DERIVED SET, READ RATHER THAN APPROXIMATED
            # (#800), and scoped to THIS mailbox like its two neighbours.
            # Production reads all three on consecutive lines of
            # ``gmail_oauth``; a harness that queried two and passed a
            # literal for the third would be measuring a function
            # production does not call. Seeding it is the board's job — an
            # offer that auto-files puts its employer in here by itself,
            # which is what lets a replay exercise the contradiction arm
            # without any fixture reaching past the entrypoint.
            contested = await employers_with_a_live_offer(session, user)
