...
async def test_an_unconfirmed_anonymous_row_holds_the_mint_back(test_session) -> None:
    ...
    THE COST THIS PARAGRAPH USED TO RECORD IS NOW CLOSED (#655), and the record
    is kept rather than deleted because the reasoning that left it open was
    wrong and somebody will re-derive it otherwise. It read: swap the two
    ``created_day`` values so the unconfirmed row is the oldest live one, it
    becomes the fold target, and ``_reopening_evidence`` walks it out of
    ``rejected`` — two rows, status ``applied``. That was true, and the stated
    reason for not fixing it was that fixing it meant steering rule 4, which
    serves review-classify and orphan-reconcile as well as the sync.

    IT DID NOT. Rule 4 still returns the same row to all three callers. What
    changed is what a rule-4 landing may LICENSE: ``_resolve_application`` now
    reports whether it picked one of SEVERAL candidates on sort order alone, and
    ``_reopening_evidence`` refuses a reopen on that landing. The swapped-day
    board is asserted in
    ``test_a_tie_break_may_not_un_reject_655.py``, both halves — the settled row
    keeps its status, and what the confirmation does instead is written down.
    """
