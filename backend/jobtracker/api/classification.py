...
# ``select(Email).where(Email.id == email_id)`` in ``classify_email``,
# ``correct_classification`` and ``approve_classification``. The
# ``user_id`` references in this file resolve a *training* identity
# (``resolve_training_user_id``); none of them scopes a row read to
# the caller.
#
