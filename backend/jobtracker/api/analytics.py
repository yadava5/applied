...
# ``get_analytics_overview`` and ``get_analytics_trends`` both read a
# bare ``select(Application)`` with no predicate at all, so every
# user's rows would be aggregated into one dashboard.
#
