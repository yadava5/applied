...
PATTERNS: dict[EmailCategory, CategoryPatterns] = {
    EmailCategory.REJECTION: CategoryPatterns(
        ...
        negative=[
            ...
            r"pdf|acrobat|lightroom|photoshop",
            r"save.{0,20}(time|money|\$|₹|%)",
            r"upgrade|discount|promo|sale",
            r"order.{0,20}(is|on its) way",
            r"unsubscribe",
            r"click here",
            r"your (order|purchase|subscription)",
        ],
    ),
    EmailCategory.INTERVIEW: CategoryPatterns(
        strong=[
            ...
            r"phone screen.{0,20}(schedule|available|call)",
    ...
    EmailCategory.OFFER: CategoryPatterns(
        strong=[
            ...
            r"formal (job )?offer",
            r"compensation package.{0,50}(salary|annual)",
    ...
    EmailCategory.APPLIED: CategoryPatterns(
        ...
        negative=[
            ...
            r"congratulations on completing",
            r"your course",
            # Marketing/promotional exclusions (kept narrow to avoid false negatives
            # on legitimate job confirmations that include generic footer text)
            r"\b(discount|promo(?:tion)?|coupon|sale|savings|limited time|flash sale)\b",
            r"\b(shop|buy|cart|checkout|purchase)\b",
            r"\byour (order|purchase|shipment)\b",
            r"\border (confirmation|number|#)\b",
            r"\btrack(ing)? your order\b",
            r"(newsletter|promotional|marketing)",
            r"most wanted",
            r"just dropped",
            r"score \$",
            r"free shipping",
            ...
            r"\bstream(ing)?\b",
            r"\btravel\b|\btour\b|\bhotel\b|\bflight\b|\bbooking\b",
            r"\birctc\b",
            r"\bgoat\.com\b",
            r"\bfubo\b",
            r"\bmyntra\b",
    ...
    EmailCategory.FOLLOW_UP: CategoryPatterns(
        strong=[
            ...
            r"wanted to (follow up|check in).{0,30}(application|interview)",
        ...
        weak=[
            r"circling back.{0,30}(application|interview|position)",
        ],
        negative=[
            r"unfortunately",
            r"offer",
            ...
            r"application.{0,20}received",
            r"order.{0,20}(is|on its) way",
            r"your (order|purchase|shipment)",
            r"tracking",
            r"delivery",
            r"survey|research|study",
            r"paid.{0,20}(study|research|survey)",
            r"unsubscribe",
        ],
    ),
}
